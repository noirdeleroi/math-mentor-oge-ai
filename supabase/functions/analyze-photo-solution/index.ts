...
serve(async (req)=>{
  ...
  try {
    ...
    function getScoringBlock(question_id, problem_number) {
      if (!question_id) return defaultBlock();
      const startsWith = (prefix)=>question_id.startsWith(prefix);
      // --- Default OGE ---
      if (startsWith("OGE")) {
        return `4. Определи итоговый числовой балл по правилам:
      ...
      // --- EGEPROF special cases ---
      if (startsWith("EGEPROF")) {
        switch(Number(problem_number)){
          case 13:
          case 15:
            return `4. Если в условии задачи ЕСТЬ два пункта а) и б), то определи итоговый числовой балл по правилам:
      ...
      Если в условии задачи НЕТ двух пунктов а) и б), то определи итоговый числовой балл по правилам:
      - **2** — решение корректное, без критичных ошибок;
      - **1** — решение доведено до конца, но есть локальная ошибка (например, арифметическая);
      - **0** — ответ неверен, решение не завершено, или ошибка фундаментальна.`;
          case 14:
      ...
      Если в условии задачи НЕТ двух пунктов а) и б), то определи итоговый числовой балл по правилам:
      - **3** — решение корректное, без критичных ошибок;
      - **2** — решение доведено до конца, но есть одна локальная ошибка (например, арифметическая);
      - **1** — решение доведено до конца, но есть две или более локальных ошибок (например, арифметические);
      - **0** — ответ неверен, решение не завершено, или ошибка фундаментальна.`;
          case 16:
            return `4. Определи итоговый числовой балл по правилам:
      - **2** — решение корректное, без критичных ошибок;
      ...
      - **3** — С помощью верного рассуждения получено множество значений $a$, отличающееся от искомого только исключением/включением точек.
      - **2** — С помощью верного рассуждения получено множество значений $a$, возможно, с включением граничных точек ИЛИ получен неверный ответ из-за вычислительной ошибки, но при этом верно выполнены все шаги решения
      - **1** — Задача верно сведена к исследованию взаимного расположения кривых И/ИЛИ и прямых (аналитически или графически)
      - **0** — Решение не соответствует ни одному из критериев, перечисленных выше`;
          case 19:
            return `4. Если в условии задачи ЕСТЬ три пункта а), б) и в), то определи итоговый числовой балл по правилам:
      ...
      - **0** — Решение не соответствует ни одному из критериев, перечисленных выше
      Если в условии задачи НЕТ трех пунктов а), б) и в), то определи итоговый числовой балл по правилам:
      - **4** — Обоснованно получен верный ответ
      - **3** — решение доведено до конца, но есть одна локальная ошибка (например, арифметическая);
      - **2** — решение доведено до конца, но есть 2-3 локальных ошибок (например, арифметические);
      - **1** — решение доведено до конца, но есть больше трех локальных ошибок (например, арифметические);
      - **0** — Решение не соответствует ни одному из критериев, перечисленных выше`;
        }
      }
      // --- Default fallback ---
...
${getScoringBlock(question_id, problem_number)}
...
- Keep the top-level shape: {"scores": <0|1|2|3|4>, "review": "<HTML>"}.
