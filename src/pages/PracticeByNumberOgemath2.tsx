...
const PracticeByNumberOgemath2 = () => {
...
// Flip UI into answered mode so button becomes "Следующий вопрос"
  ...
  const handleDevicePhotoCheck = async () => {
    ...
    // Show the sci-fi scanning dialog
  ...
  return (
    <div className="min-h-screen md:min-h-screen text-white relative w-full max-w-full overflow-x-hidden" style={{ background: "linear-gradient(135deg, #1a1f36 0%, #2d3748 50%, #1a1f36 100%)" }}>
            ...
            <>
              ...
              <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                <h2 className="text-lg sm:text-xl md:text-2xl font-display font-bold bg-gradient-to-r from-yellow-500 to-emerald-500 bg-clip-text text-transparent">
                  Вопрос {currentQuestionIndex + 1} из {currentQuestion?.problem_number_type === 20 ? 602 : questions.length}
                </h2>
      ...
      {/* Sci-Fi Scanning Dialog with Image Preview */}
      <Dialog open={showScanningDialog && isProcessingPhoto && uploadedImages.length > 0} onOpenChange={() => {}}>
        <DialogContent className="sm:max-w-2xl [&>button]:hidden p-0 overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border-yellow-500/30">
          <div className="relative">
            {/* Image Container with Sci-Fi Effects */}
            <div className="relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <div 
                  ...
                  style={{
                    background: 'linear-gradient(to bottom, transparent 0%, rgba(234, 179, 8, 0.3) 50%, transparent 100%)',
                ...
                <div className="absolute top-0 left-0 w-16 h-16 border-t-2 border-l-2 border-yellow-500 opacity-60" />
                <div className="absolute top-0 right-0 w-16 h-16 border-t-2 border-r-2 border-yellow-500 opacity-60" />
                <div className="absolute bottom-0 left-0 w-16 h-16 border-b-2 border-l-2 border-emerald-500 opacity-60" />
                <div className="absolute bottom-0 right-0 w-16 h-16 border-b-2 border-r-2 border-emerald-500 opacity-60" />

                {/* Glowing Border Effect */}
                <div className="absolute inset-0 pointer-events-none">
                  <div className="absolute inset-0 border-2 border-yellow-500/50 shadow-[0_0_20px_rgba(234,179,8,0.5)] animate-pulse" />
                </div>
                  ...
                  {[...Array(6)].map((_, i) => (
                    <div
                      ...
                      className={`absolute w-1 h-1 rounded-full opacity-60 ${
                        i % 2 === 0 ? 'bg-yellow-500' : 'bg-emerald-500'
                      }`}
                      style={{
                        left: `${20 + i * 15}%`,
                        top: `${30 + (i % 3) * 20}%`,
            ...
            <div className="absolute bottom-4 left-0 right-0 text-center z-10">
              <div className="inline-block bg-black/70 backdrop-blur-sm px-6 py-3 rounded-lg border border-yellow-500/50">
                <div className="flex items-center justify-center gap-3">
                  <div className="w-2 h-2 bg-yellow-500 rounded-full animate-pulse" />
                  <span className="text-yellow-400 font-mono text-sm tracking-wider">
                    {ocrProgress || 'Сканирование решения...'}
                  </span>
            ...
            <div className="bg-black/80 backdrop-blur-sm border-t border-yellow-500/30 p-4 space-y-3">
              {/* Upload Progress */}
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="text-yellow-400 font-mono">[ЗАГРУЗКА]</span>
                  <span className="text-emerald-400 font-mono">{Math.round(uploadProgress)}%</span>
                </div>
                <div className="w-full bg-gray-700/50 rounded-full h-1.5 overflow-hidden border border-yellow-500/20">
                  <div
                    className="h-full bg-gradient-to-r from-yellow-500 to-emerald-500 rounded-full transition-all duration-300 ease-out shadow-[0_0_10px_rgba(234,179,8,0.5)]"
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="text-yellow-400 font-mono">[АНАЛИЗ]</span>
                  <span className="text-emerald-400 font-mono">{Math.round(analysisProgress)}%</span>
                </div>
                <div className="w-full bg-gray-700/50 rounded-full h-1.5 overflow-hidden border border-yellow-500/20">
                  <div
                    className="h-full bg-gradient-to-r from-yellow-500 to-emerald-500 rounded-full transition-all duration-300 ease-out shadow-[0_0_10px_rgba(234,179,8,0.5)]"
          ...
          <style>{`
            ...
            @keyframes float3 {
              0%, 100% { transform: translateY(0) translateX(0); opacity: 0.6; }
              50% { transform: translateY(-9px) translateX(-2px); opacity: 1; }
            }
            @keyframes float4 {
              0%, 100% { transform: translateY(0) translateX(0); opacity: 0.6; }
              50% { transform: translateY(-11px) translateX(3px); opacity: 1; }
            }
            @keyframes float5 {
              0%, 100% { transform: translateY(0) translateX(0); opacity: 0.6; }
              50% { transform: translateY(-7px) translateX(-4px); opacity: 1; }
            }
          `}</style>
        </DialogContent>
      </Dialog>
...
export default PracticeByNumberOgemath2;
