...
async function generatePages() {
  try {
    const template = readFileSync(join(__dirname, 'template-egemathprof.html'), 'utf-8');
    const outputDir = join(__dirname, '../../../public/questionbankegep');
    const sitemapUrls = [];
    ...
    for (const { label, number } of questionTypes) {
      ...
      try {
        ...
        const fetchPromise = supabase.from('egemathprof').select('*').eq('problem_number_type', number);
     ...
     `;
        ...
        sitemapUrls.push(`/questionbankegep/${label}/`);
