...
const EgemathprofMock = () => {
  ...
  // Photo upload states for problems 13-19
  ...
  const isPhotoQuestion = currentQuestion?.problem_number_type && currentQuestion.problem_number_type >= 13;
  ...
  const generateQuestionSelection = async () => {
    ...
    try {
      const batchPromises = [];
      for (let problemNum = 1; problemNum <= 19; problemNum++) {
        batchPromises.push(
          supabase
            .from('egemathprof')
            .select('*')
      ...
      batchResults.forEach((result, index) => {
        ...
        if (result.error) {
          console.error(`Error fetching problem ${problemNum}:`, result.error);
          missingProblems.push(problemNum);
      ...
      if (selectedQuestions.length < 19) {
        console.error(`Missing questions for problem numbers: ${missingProblems.join(', ')}`);
        console.error(`Only loaded ${selectedQuestions.length} out of 19 questions`);
        toast.error(`Не удалось загрузить все вопросы экзамена. Загружено: ${selectedQuestions.length}/19. Отсутствуют номера: ${missingProblems.join(', ')}`);
        return;
      }

      console.log('Pre-fetching question details for all 19 questions...');
      const questionDetailsPromises = allQuestionIds.slice(0, 19).map(questionId =>
      ...
      setQuestions(selectedQuestions.slice(0, 19));
      const initialExamResults: ExamResult[] = selectedQuestions.slice(0, 19).map((question, index) => ({
        questionIndex: index,
...
const handleStartExam = async () => {
  ...
  // --- NEW: insert placeholder for Part 2 (13–19) so results always see an attempted row
  ...
  const handleNextQuestion = async () => {
    ...
    try {
      ...
      if (user) {
        if (userAnswer.trim()) {
          if (problemNumber >= 13) {
            // FRQ (13–19) — INSERT placeholder then fire-and-forget analysis
            try {
              const { data: profile } = await supabase
                .from('profiles')
                .select('exam_id')
            ...
            // Part 1 (1–12) — INSERT first, then possibly server-check and UPDATE openrouter_check ("true"/"false")
          ...
          try {
            ...
            await supabase
              ...
              .insert({
                ...
                problem_number: problemNumber.toString(),
                raw_output: 'False',
                analysis_type: problemNumber >= 13 ? 'photo_solution' : 'solution',
      ...
      if (filteredResults.length > 0) {
        for (const analysisResult of filteredResults) {
          ...
          if (problemNumber >= 13) {
            ...
            // Part 1 (1–12) — prefer openrouter_check if present ("true"/"false" as TEXT)
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-6">
              <h1 className="text-3xl md:text-4xl font-display font-bold bg-gradient-to-r from-yellow-500 to-emerald-500 bg-clip-text text-transparent">
                Пробный экзамен ЕГЭ Профильная математика
              </h1>
            ...
            <Card className="bg-white/10 backdrop-blur border border-white/20 rounded-2xl shadow-xl">
              <CardContent className="p-6">
                <div className="space-y-4 text-white/80 mb-6">
                  <p>• <strong className="text-white">19 вопросов</strong> в порядке от 1 до 19</p>
                  <p>• <strong className="text-white">Время:</strong> 3 часа 55 минут (таймер запускается автоматически)</p>
                  <p>• <strong className="text-white">Вопросы 1-12:</strong> текстовые ответы</p>
                  <p>• <strong className="text-white">Вопросы 13-19:</strong> развернутые решения с фото</p>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <Card className="bg-white/95 backdrop-blur border border-white/20 rounded-2xl shadow-xl">
                <CardHeader><CardTitle className="text-[#1a1f36]">Часть 1 (1-12)</CardTitle></CardHeader>
                <CardContent>
              ...
              <Card className="bg-white/95 backdrop-blur border border-white/20 rounded-2xl shadow-xl">
                <CardHeader><CardTitle className="text-[#1a1f36]">Часть 2 (13-19)</CardTitle></CardHeader>
                <CardContent>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3 text-white/80">
              ...
              <Button
                onClick={() => toast.info('Ты сдаёшь профильный уровень — никаких формул для тебя 😜')}
                variant="outline"
                className="
                  bg-transparent
...
export default EgemathprofMock;
