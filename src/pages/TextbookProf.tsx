...
import egeProfSyllabusData from '../data/egeProfSyllabusStructure.json';
...
const getAllSkillsFromStructure = (): Skill[] => {
  ...
  const syllabusData = egeProfSyllabusData as SyllabusStructure;
...
const TextbookProf = () => {
  ...
  const handleSendChatMessage = async (userInput: string) => {
    ...
    try {
      ...
      await saveChatLog(userInput, aiResponse.text, COURSE_ID);
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="font-display text-3xl font-bold mb-2 bg-gradient-to-r from-yellow-500 to-emerald-500 text-transparent bg-clip-text">
            Программа ЕГЭ по математике (Профиль)
          </h1>
        </div>
