...
export default function PhotoAnalysisFeedback({
  ...
  const wrapLatex = (text: string): string => {
    if (!text) return text;
    // already delimited?
    if (text.includes('$$') || text.trim().startsWith('$') || /\\\(|\\\)|\\\[|\\\]/.test(text)) {
      return text;
    }
    // looks like TeX?
  ...
  const highlightText = (text: string, snippets: string[]): React.ReactNode[] => {
    ...
    nonOverlappingMatches.forEach((match) => {
      if (match.start > currentIndex) {
        parts.push(
          <span key={`text-${currentIndex}`}>{text.substring(currentIndex, match.start)}</span>
        );
      }
      ...
      parts.push(
        <span
          ...
          style={{ backgroundColor: isActive ? "#fff700" : "rgba(255, 247, 0, 0.4)" }}
        >
          {match.snippet}
        </span>
      );
          ...
          {scores !== undefined && scores !== null && (
            <div className="mb-4 p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border-2 border-green-300">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-green-800">Оценка:</span>
                <div className="flex items-center gap-2">
                  <span className="text-lg font-bold text-green-700">{scores}/2</span>
                  <div className="flex gap-1">
                    {[1, 2].map((score) => (
                      <div
                        key={score}
                        className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                          score <= scores ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-500'
                        }`}
                      >
                        ✓
                      </div>
      ...
      <div className="space-y-4 max-h-[500px] overflow-y-auto">
        ...
        {analysisData.review.errors.map((error, index) => (
          <Card
            ...
            className={`${ERROR_TYPE_COLORS[error.type] || "border-gray-500 bg-gray-50"} transition-all duration-300 ${
              activeErrorIndex === index ? "ring-2 ring-yellow-400 shadow-lg" : "hover:shadow-md"
            }`}
            onClick={() => handleErrorCardClick(index)}
          >
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-medium">Ошибка {index + 1}</CardTitle>
                <div className="flex gap-2 items-center">
                  {error.severity && (
                    <Badge variant={error.severity === 'major' ? 'destructive' : 'secondary'}>
            ...
            <CardContent className="space-y-3">
              <div>
                <p className="text-xs font-medium text-gray-600 mb-2">Ваше решение:</p>
                <div className="bg-white p-3 rounded border">
                  <MathRenderer
                    key={`err-stu-${index}-${(error.student_latex || '').length}`}
                    text={wrapLatex(error.student_latex)}
                    compiler="mathjax"
                  />
                </div>
              </div>
              ...
              <div>
                <p className="text-xs font-medium text-gray-400 mb-2">Правильное решение:</p>
                <div className="bg-gray-100 p-3 rounded border border-gray-300">
                  <MathRenderer
                    key={`err-exp-${index}-${(error.expected_latex || '').length}`}
                    text={wrapLatex(error.expected_latex)}
                    compiler="mathjax"
                  />
                </div>
              </div>
              ...
              {error.explanation_latex && (
                <div className="bg-blue-50 p-3 rounded border border-blue-200 mt-3">
                  <p className="text-xs font-medium text-blue-700 mb-2">Объяснение:</p>
                  <MathRenderer
                    key={`err-expl-${index}-${(error.explanation_latex || '').length}`}
                    text={wrapLatex(error.explanation_latex)}
                    compiler="mathjax"
                  />
                </div>
              )}
              ...
              {error.suggested_fix_latex && (
                <div className="bg-green-50 p-3 rounded border border-green-200 mt-3">
                  <p className="text-xs font-medium text-green-700 mb-2">Рекомендация:</p>
                  <MathRenderer
                    key={`err-fix-${index}-${(error.suggested_fix_latex || '').length}`}
                    text={wrapLatex(error.suggested_fix_latex)}
                    compiler="mathjax"
                  />
                </div>
              )}
